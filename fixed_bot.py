...
def get_xml_url(filing_url: str) -> Optional[str]:
    try:
...
def parse_form4_xml(xml_url: str) -> Optional[Dict]:
    try:
        ...
        for tx in soup.find_all("nonDerivativeTransaction"):
            ...
            buys.append({"date": date_txt, "shares": shares, "price": price, "value": value})
...
def format_alert(data: Dict, filing_url: str) -> str:
    lines = [
        ...
        f"Insider: {data.get('owner_name', '')}" + (f" — {data.get('owner_title')}" if data.get("owner_title") else ""),
        f"Total Buy Value: {format_money(data.get('total_value', 0))}",
        "",
        "Transactions:",
    ]

    for b in data.get("buys", [])[:5]:
        lines.append(f"- {b['date']}: {b['shares']:,.0f} shares @ ${b['price']:,.2f} = {format_money(b['value'])}")
...
def main() -> None:
    ...
    send_telegram("✅ Insider alert bot is running.")

    seen = load_seen()

    while True:
        try:
            logging.info("Checking SEC Form 4 feed...")
            ...
            for entry in reversed(feed.entries):
                ...
                xml_url = get_xml_url(filing_url)
                seen.add(entry_id)
                ...
                send_telegram(format_alert(data, filing_url))
